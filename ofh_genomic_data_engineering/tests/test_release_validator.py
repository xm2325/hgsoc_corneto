...
def _write_release(tmp_path: Path) -> dict[str, Path]:
    ...
    tables = {
        ...
        "variant_missingness": pd.DataFrame(
            {
                "CHROM": ["22", "22"],
                "ID": ["rs1", "rs2"],
                "MISSING_CT": ["0", "0"],
        ...
        "hardy_weinberg": pd.DataFrame(
            {"CHROM": ["22", "22"], "ID": ["rs1", "rs2"], "P": ["1", "0.75"]}
        ),
        "pca_scores": pd.DataFrame(
            {"FID": ["S1", "S2"], "IID": ["S1", "S2"], "PC1": ["0.1", "-0.1"]}
        ),
    }
...
def test_release_contract_rejects_duplicate_sample_ids(tmp_path: Path) -> None:
    ...
    samples["IID"] = ["S1", "S1"]
    samples.to_parquet(paths["parquet_dir"] / "samples.parquet", index=False)

    provenance = json.loads(paths["provenance_path"].read_text())
    changed = paths["parquet_dir"] / "samples.parquet"
    provenance["products"][changed.name]["sha256"] = sha256(changed)
